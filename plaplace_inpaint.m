function [ x, u, num_of_iter ] = plaplace_inpaint( img,fillImg,fillRegion )

display('.......................')
disp('P-Laplace Method')
display('.......................')
...
Unn=(Ux2.*Uxx)+(2.*Ux.*Uy.*Uxy)+(Uy2.*Uyy);
Unn=Unn./den;

Dfull=((magU.^(-0.5)).*Uzz)+(0.5.*(magU.^(-0.5)).*Unn);
% Dfull=sum(Dfull,3);
Dfull=double(rgb2gray(uint8(Dfull)));
